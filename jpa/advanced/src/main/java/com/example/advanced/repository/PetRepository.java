...
public interface PetRepository extends JpaRepository<Pet, Long>, PetCustomRepository {
    ...
    @Query("select p from Pet p join fetch p.owner")    // Pet의 p를 조회할 때 pet과 연관관계에 있는 owner까지 한꺼번에 조회해옴
    public List<Pet> findAllFetchJoin();

    @Override
    @EntityGraph(attributePaths = "owner")  // fetchJoin하고 싶은 객체를 써줌, 객체의 Graph를 찾으면 어디까지 참조할 수 있게 해줄까?라는 영역을 정해주는 것
    public List<Pet> findAll();

    @EntityGraph(attributePaths = "owner")  // EntityGraph를 쓸 때에는 무조건 @Query가 있어야함, findAll()은 이미 @Query를 내포하고 있으므로 생략이 가능
